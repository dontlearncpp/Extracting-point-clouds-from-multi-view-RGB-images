#--trained_model=output/yolact_coco_custom_103_8500.pth --score_threshold=0.15 --top_k=15 --images=/media/xingshixu/367a0adf-4bec-4c64-b23d-945aacb28ba5/yang/yolact++_resnet_ecam_cafpn_res/yolact/yolact-master/images:/media/xingshixu/367a0adf-4bec-4c64-b23d-945aacb28ba5/yang/yolact++_resnet_ecam_cafpn_res/yolact/yolact-master/images_out
...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str='', save_path=''):
    ...
    zuizhong_path = "/media/xingshixu/367a0adf-4bec-4c64-b23d-945aacb28ba5/yang/yolact++_resnet_ecam_cafpn_res/yolact/yolact-master/black_white/" + str(path[1])
    zuizhong_txt = "/media/xingshixu/367a0adf-4bec-4c64-b23d-945aacb28ba5/yang/yolact++_resnet_ecam_cafpn_res/yolact/yolact-master/txt/" + str(path[1])
    # print("ZuiZhong_path is {}".format(zuizhong_path))
    ...
    with timer.env('Postprocess'):
        ...
        for i in range(len(t[3])):
            ...
            duijixian=[]
            imag = np.array(t[3][i,:,:].cpu())

            testtt = imag.flatten()
            tttt = testtt[np.argmax(testtt)]
            lujing = zuizhong_path.rsplit(".")
            lujing1 =zuizhong_txt.rsplit(".")
            zuizhong_txt = str(lujing1[0])+".txt"
            #     print("lujing ")
            # zuizhong_path = lujing[0]
            if i!=0:
                zuizhong_path = str(lujing[0])+"_"+str(i)+".png"
                zuizhong_txt = str(lujing[0])+"_"+str(i)+".txt"
            imag *= 255
            # cv2.imshow('imag',imag)
            cv2.imwrite(zuizhong_path, imag)
            imag *=255
            imag = Image.fromarray(imag)
            gray = imag.convert('L')
            gray = np.array(gray)
            # gray = cv2.cvtColor(imag,cv2.COLO)
            ret,binary = cv2.threshold(gray,127,255,0)
            contours,hiearchy = cv2.findContours(binary,cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)
            for k in range(len(contours)):
                # mm = contours[k].shape[0]
                # hull1.append(mm)
                dian_list=list(contours[k])
                dianx = []
                diany = []
                #sss=contours[k].shape[0]
                for di in range(contours[k].shape[0]):
                    dianx.append(contours[k][di][0,0])
                    diany.append(contours[k][di][0,1])

                # duijiaoxian.append(contours[k])
                dianx_max = max(dianx)
                dianx_min = min(dianx)
                diany_max = max(diany)
                diany_min = min(diany)
                diss = (dianx_max-dianx_min)**2+(diany_max-diany_min)**2
                duijixian.append(diss)
                #hull1.append(contours[k].shape[0])
            max_con=max(duijixian)
            aa = duijixian.index(max_con)
            # if len(contours) == 1:
            #     aa=0
            #hull = cv2.convexHull(contours[aa][:,:])
            hull = contours[aa][:,:]
            hullf=hull.flatten()#zhanping
            file = open(zuizhong_txt, 'w')
            for i in range(hullf.size):
                file.write(str(hullf[i])+"\n")
            file.close()
...
def evalimage(net: Yolact, path: str, save_path: str = None):
    ...
    img_numpy = prep_display(preds, frame, None, None, undo_transform=False, save_path=save_path)
